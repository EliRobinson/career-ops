...
<script/><!-- EDUCATION --><div class="section-title">Education</div><!-- SKILLS --><div class="section-title">Skills</div></script>
  ...
  //
  // The expected result is the reversed KEY list mapped to titles and dropped
  // into the template's nine slot positions in document order — not simply
  // shippedOrder reversed. reorderCvSections permutes named sections among the
  // slots they already occupy (see its docstring), and cv-template.html renders
  // Skills above Education rather than last, so its shipped order does not match
  // allKeys' order and the two reversals are not interchangeable here.
  ...
  const keyTitle = (key) => RENDERED_TITLES[`SECTION_${key.toUpperCase()}`];
  const reversed = renderedTitles(reorderCvSections(rendered, [...allKeys].reverse()));
  const expectedReversed = [...allKeys].reverse().map(keyTitle);
  if (shippedOrder.length === 9 && JSON.stringify(reversed) === JSON.stringify(expectedReversed)) {
