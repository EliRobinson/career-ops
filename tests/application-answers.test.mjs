...
try {
  const {
    ...
    parseApplicationAnswersSnapshot,
  ...
  for (const { name, snapshot } of corpus) {
    const gen1 = parseApplicationAnswersSnapshot(formatApplicationAnswersSection(snapshot));
    const gen2 = parseApplicationAnswersSnapshot(formatApplicationAnswersSection(gen1));
    observedEntries +=
  ...
  const parsed = parseApplicationAnswersSnapshot(
  ...
  const empty = parseApplicationAnswersSnapshot(
  ...
  const bounded = parseApplicationAnswersSnapshot(trailing);
  ...
  if (
    parseApplicationAnswersSnapshot(report) === null &&
    parseApplicationAnswersSnapshot('') === null &&
    parseApplicationAnswersSnapshot(undefined) === null
  ) {
  ...
  const lenient = parseApplicationAnswersSnapshot(mangled);
  let strictThrew = null;
  try {
    parseApplicationAnswersSnapshot(mangled, { strict: true });
  } catch (e) {
    strictThrew = e.message;
  }
  let strictOnCleanThrew = null;
  try {
    parseApplicationAnswersSnapshot(clean, { strict: true });
  } catch (e) {
  ...
  const lenientHead = parseApplicationAnswersSnapshot(freeTextHeadMangled);
  const lenientMid  = parseApplicationAnswersSnapshot(freeTextMidMangled);

  let freeTextStrictThrew = null;
  try {
    parseApplicationAnswersSnapshot(freeTextHeadMangled, { strict: true });
  } catch (e) {
    freeTextStrictThrew = e.message;
  }
  let midStrictThrew = null;
  try {
    parseApplicationAnswersSnapshot(freeTextMidMangled, { strict: true });
  } catch (e) {
  ...
  try {
    ...
    cliChecks.push([
      cleanOut !== null &&
        ...
        JSON.stringify(parseApplicationAnswersSnapshot(clean)),
    ...
    cliChecks.push([
      lenientOut !== null &&
        ...
        JSON.stringify(parseApplicationAnswersSnapshot(mangled)),
