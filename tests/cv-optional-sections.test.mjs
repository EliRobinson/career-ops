...
// Education to match cv.md's order), so it may have no following section marker
...
// section's marker when a custom template puts Skills higher up (as
// cv-template.html itself does). That produces two behaviours this suite pins
// down:
//
