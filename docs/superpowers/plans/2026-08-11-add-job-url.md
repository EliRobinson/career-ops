...
- **Prompt freeze (`#2185`):** ~~`test-all.mjs` asserts on the exact string returned by `buildPrompt`.~~ **This was wrong, and is corrected here rather than quietly edited out, because Task 2 was designed around it.** `test-all.mjs` has no assertion on the evaluate prompt: the `#2185` freeze covers `claudeCliArgs` argv and tool scopes, and `test-all.mjs:13213` states that the prompt is left to `run-prompts.test.mjs`. Task 2's `mirrorClause` collapsing to `""` is still the right shape (it matches the existing `mem` clause), but it was a free design choice, not an external constraint. Nothing stops a future edit from rewording the prompt.
