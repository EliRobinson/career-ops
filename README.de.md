...
pnpm exec playwright install chromium   # nur für PDF-Generierung nötig

# 2. Setup prüfen
pnpm run doctor                     # validiert alle Voraussetzungen
...
Standardmäßig vertraut `node scan.mjs` (alias `pnpm run scan`) den Rückgaben der ATS-Feeds. Einige Unternehmen lassen alte Stellenanzeigen öffentlich verfügbar, obwohl Rollen bereits geschlossen sind. Mit `--verify` startet Playwright nach dem API-Lauf und entfernt abgelaufene Stellenanzeigen, bevor sie in `pipeline.md` landen:
