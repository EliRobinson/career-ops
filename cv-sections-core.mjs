...
// somewhere else (or, in cv-template.html, when Skills precedes Education).
// Matching the sentinel *only* would be wrong for that second case: the lazy
// body would run past every section between Skills and the sentinel and delete
// them along with the empty header, which is silent data loss in a populated
// CV. Two consequences, both intentional:
//
...
// Skills is last and at the next section's marker when it is not (or, in
// cv-template.html, at Education's marker, since Skills renders before it
// there). See "The Skills sentinel" above before changing these, and never add
// an `|$` branch.
//
